...
export default function CandidatsPage() {
  ...
  async function startDiscussion(developerId: string) {
    const { data: { session } } = await supabase.auth.getSession();
    const res = await fetch("/api/messages/start", {
      method: "POST",
      headers: { "Content-Type": "application/json", Authorization: `Bearer ${session?.access_token ?? ""}` },
      body: JSON.stringify({ projectId: id, developerId }),
    });
            ...
            <div className="flex flex-col gap-3">
              {pending.map((c) => (
                <CandidatCard
                  ...
                  onDiscuss={() => startDiscussion(c.profiles_developer.id)}
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex gap-2 flex-wrap">
            ...
            {onDiscuss && (
              <button
                onClick={onDiscuss}
                style={{ display: "inline-flex", alignItems: "center", gap: 6, padding: "8px 16px", borderRadius: 10, border: "1px solid #E5E5EA", background: "#fff", color: "#1A2138", fontSize: 13, fontWeight: 600, cursor: "pointer" }}
              >
                Discuter
              </button>
            )}
