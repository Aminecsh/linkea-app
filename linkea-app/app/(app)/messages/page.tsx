...
import { STAGGER_LIMIT } from "@/lib/motionVariants";
...
export default function MessagesPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      if (r === "founder") {
        ...
        if (profile) {
          const { data } = await supabase
            .from("conversations")
            .select("id, created_at, project_id, developer_id, projects(titre, statut), profiles_founder(nom, user_id), profiles_developer(nom, user_id)")
      ...
      // Masquer les discussions liées à une candidature refusée
      const { data: cands } = enriched.length > 0
        ? await supabase.from("candidatures").select("project_id, developer_id, statut")
            .in("project_id", enriched.map((c) => c.project_id))
        : { data: [] };
      const refusedKeys = new Set(
        (cands ?? []).filter((c) => c.statut === "refused").map((c) => `${c.project_id}_${c.developer_id}`)
      );
      const visible = enriched.filter((c) => !refusedKeys.has(`${c.project_id}_${c.developer_id}`));
  ...
  const enCours      = conversations.filter((c) => ["matched", "en_cours"].includes(c.projects?.statut));
  const enDiscussion = conversations.filter((c) => c.projects?.statut === "pending");
  const archived      = conversations.filter((c) => ARCHIVED.includes(c.projects?.statut));
  const totalUnread = [...enCours, ...enDiscussion].reduce((sum, c) => sum + c.unreadCount, 0);

  function ConvCard({ c, isArchived, index = 0 }: { c: Conversation; isArchived?: boolean; index?: number }) {
    ...
    return (
      <motion.div
        ...
        transition={{
          ...
          ease: "easeOut",
          delay: reduceMotion ? 0 : Math.min(index, STAGGER_LIMIT) * 0.05,
        }}
  ...
  return (
    <div className="min-h-screen pb-nav pl-sidebar" style={{ background: "var(--bg)" }}>
      ...
      <div className="max-w-2xl mx-auto px-4 py-4 flex flex-col gap-2">
        ...
        {/* État vide */}
        {enCours.length === 0 && enDiscussion.length === 0 && archived.length === 0 ? (
          <div
            ...
            {enCours.length > 0 && (
              <div className="flex flex-col gap-2">
                <div className="flex items-center gap-2 py-2 px-1 mb-1">
                  ...
                  <span className="label">{enCours.length}</span>
                </div>
                {enCours.map((c, i) => <ConvCard key={c.id} c={c} index={i} />)}
              </div>
            )}

            {/* Candidatures en discussion — avant match */}
            {enDiscussion.length > 0 && (
              <div className="flex flex-col gap-2 mt-2">
                <div className="flex items-center gap-2 py-2 px-1 mb-1">
                  <span className="label">Candidatures en discussion</span>
                  <div className="flex-1 h-px" style={{ background: "var(--border)" }} />
                  <span className="label">{enDiscussion.length}</span>
                </div>
                {enDiscussion.map((c, i) => <ConvCard key={c.id} c={c} index={i} />)}
              </div>
            )}
              ...
              <div className="mt-2">
                ...
                {archivesOpen && (
                  <div className="flex flex-col gap-2 opacity-75">
                    {archived.map((c, i) => <ConvCard key={c.id} c={c} isArchived index={i} />)}
                  </div>
                )}
              </div>
            )}
