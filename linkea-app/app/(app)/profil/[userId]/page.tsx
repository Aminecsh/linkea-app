...
export default function PublicProfilePage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      if (roleData.role === "developer") {
        // Profil public — servi par une route cachée 5 min (identique pour tous les visiteurs)
        const prof: DevProfile | null = await fetch(`/api/dev-profiles/${userId}`)
  ...
  async function startDiscussion(projectId: string) {
    const { data: { session } } = await supabase.auth.getSession();
  ...
  async function revalidateDevProfileCache() {
    const { data: { session } } = await supabase.auth.getSession();
    if (!session?.access_token) return;
    fetch(`/api/dev-profiles/${userId}`, {
      method: "POST",
  ...
  async function handleAvatarChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (!error) {
      ...
      if (targetRole === "developer") { setDevProfile((p) => p ? { ...p, avatar_url: publicUrl } : p); revalidateDevProfileCache(); }
                  ...
                  {myDevCandFiltered.map((cand) => {
                    ...
                    return (
                      <div key={cand.id} style={{ background: C.surface, border: `1.5px solid ${C.hairline}`, borderRadius: 14, padding: "16px 18px" }}>
                        ...
                        <div style={{ paddingTop: hasContract ? 8 : 10, display: "flex", gap: 14 }}>
                          ...
                          {cand.statut !== "refused" && (
                            <button onClick={() => startDiscussion(cand.project_id)}
                              style={{ background: "none", border: "none", cursor: "pointer", fontSize: 12, fontWeight: 600, color: C.ink, display: "flex", alignItems: "center", gap: 4 }}>
                              <MessageCircle size={12} strokeWidth={2} /> Discuter
                            </button>
                          )}
                        </div>
                      </div>
