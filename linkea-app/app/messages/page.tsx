    ...
    return (
      <div
        ...
        className={`bg-white rounded-2xl border-2 p-4 flex items-center gap-4 cursor-pointer transition-all ${
          isArchived
            ? "border-slate-100 opacity-75 hover:opacity-100 hover:border-slate-200"
            : hasUnread
      ...
      >
        <div className="relative shrink-0">
          ...
          {isArchived && (
            <span className="absolute -bottom-1 -right-1 w-5 h-5 bg-slate-400 text-white text-xs rounded-full flex items-center justify-center border border-white">
              📦
            </span>
          )}
        </div>
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 flex-wrap">
            <p className={`font-bold ${isArchived ? "text-slate-500" : "text-slate-900"} ${hasUnread ? "text-pink-600" : ""}`}>
              {otherNom ?? "—"}
            </p>
